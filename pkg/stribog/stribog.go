...
//	}
//	var hash chunk
//	streebogCore(message, &hash)
//
//	//fmt.Printf("Message (hex): %x\n", message)
//	//fmt.Printf("Message (ASCII): %s\n", message[53:])
//	//fmt.Printf("Streebog-512: %02x\n", hash)
//}

func add512(a, b, result *chunk) {
	var temp uint16 = 0
	for i := 63; i >= 0; i-- {
		temp = uint16(a[i]) + uint16(b[i]) + (temp >> 8)
		result[i] = byte(temp)
	}
}

func transformX(a, b, out *chunk) {
	for i := 0; i < CELLS; i++ {
		out[i] = a[i] ^ b[i]
	}
}

func transformS(input *chunk) {
	for i, val := range input {
		input[i] = PI[val]
	}
}

func transformP(input *chunk) {
	var temp = *input
	for i := range TAU {
		input[i] = temp[TAU[i]]
	}
...
func transformL(input *chunk) {
	var input64 [8]uint64
	var buffer [8]uint64
	for i := 0; i < CELLS; i += 8 {
		input64[i/8] = binary.BigEndian.Uint64(input[i : i+8])
	}
	for i := 0; i < 8; i++ {
		for j := 0; j < 64; j++ {
			if (input64[i]>>j)&1 == 1 {
	...
	for i := range input64 {
		binary.BigEndian.PutUint64(input[i*8:i*8+8], buffer[i])
	}
}

func keySchedule(keys *chunk, iterIndex int) { // ok
	keysCopy := *keys
	transformX(&keysCopy, &C[iterIndex], keys)
	transformS(keys)
	transformP(keys)
	transformL(keys)
}

func transformE(keys, block, state *chunk) {
	...
	for i := 0; i < 12; i++ {
		...
		keySchedule(keys, i)
		stateCopy := *state
		transformX(keys, &stateCopy, state)
	}
}

func transformG(n, hash, message *chunk) {
	var keys chunk
	var temp chunk
	...
	//fmt.Printf("temp %02x\n", temp)
	tempClone := temp
	transformX(&tempClone, hash, &temp)
	transformX(&temp, message, hash)
	//fmt.Printf("hash %02x\n", hash)
}

func streebogCore(message []byte, hash *chunk) {
	var n chunk
	var sigma chunk
	var blockSize chunk
	var block chunk

	// Обрабатываем сообщение блоками по 64 байта
	for i := 0; i < len(message); i += 64 {
		...
		// Вычисление размера полезных данных блока (в битах)
		chunkSize := uint16(len(piece)) * 8
		binary.BigEndian.PutUint16(blockSize[62:], chunkSize)

		// Дополнение блока - В НАЧАЛО!
		if len(piece) != 64 {
			block = chunk{}
			// Копируем данные в конец блока, оставляя начало пустым
			offset := 64 - len(piece)
			copy(block[offset:], piece)
			block[offset-1] = 1 // Устанавливаем бит дополнения в начало данных
		} else {
			copy(block[:], piece)
		}
		//fmt.Printf("block %02x\n", block)
		//reverseBytes(block[:])
		////fmt.Printf("block reverse %02x\n", block)

		transformG(&n, hash, &block)
		//fmt.Printf("transformG %02x\n", hash)

		nCopy := n
		add512(&nCopy, &blockSize, &n)

		sigmaCopy := sigma
		add512(&sigmaCopy, &block, &sigma)
...
// Вспомогательная функция для разворота байтов
